series24.events.on("validated", function() {
    ...
    } else if (currentES >= up2 - 1 && currentES <= up2 + 2) {
        fetch('https://v0s00gsowwg0wow44o0884ss.rpa.chapilabs.com/api/data', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            ...
            body: JSON.stringify({
                values: {
                    flow_weak_up_level: up2,
                    currentES: currentES
                }
            }),
        }).then(response => {
            console.log("Flow Level message sent!");
        }).catch(error => {
            console.error("Error sending Flow Level message:", error);
        });
    } else if (currentES <= dn2 + 1 && currentES >= dn2 - 2) {
        fetch('https://v0s00gsowwg0wow44o0884ss.rpa.chapilabs.com/api/data', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
